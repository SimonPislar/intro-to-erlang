...
%% @doc The hello function. Prints N number of "hello" and 1 ok.
...
char_to_upper(Char) when Char > 96 -> Char - 32;
char_to_upper(Char) when Char =< 96 -> Char.
...
char_to_lower(Char) when Char < 91 -> Char + 32;
char_to_lower(Char) when Char >= 91 -> Char.
...
str_to_upper(String) -> [char_to_upper(C) || C <- String].
...
str_to_lower(String) -> [char_to_lower(C) || C <- String].
...
max([H | T]) ->
    lists:foldl(fun(Max, Acc) -> if Max > Acc -> Max ; true -> Acc end end, H, T).
...
count(String, Char) ->
  lists:foldl(fun(C, Acc) -> if C == Char -> Acc + 1 ; true -> Acc end end, 0, String).
